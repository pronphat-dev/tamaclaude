...
Command Line Tools พอแล้ว · บนเครื่องที่ SDK ใหม่สุดเป็น 27 ขึ้นไป `@State` เป็น macro
และปลั๊กอินที่ขยายมันมากับ Xcode.app เท่านั้น — `make-app.sh` จึงลอง SDK ที่มีในเครื่อง
แล้วเลือกตัวใหม่ที่สุดที่ยังคอมไพล์ SwiftUI ได้ พร้อมบอกว่าเลือกตัวไหน ถ้าไม่มีตัวไหนได้เลย
มันจะบอกตรงๆ ว่าต้องใช้ Xcode แทนที่จะปล่อยให้สาเหตุจมอยู่ใต้ error เป็นร้อยบรรทัด
