...
public final class Daemon {
    ...
    private func settle(_ id: String, allow: Bool?) {
        ...
        // มีคนตอบแล้วจริง — สถานะที่ค้างจาก `PermissionRequest` ต้องจบลงที่นี่ ไม่ใช่รอ
        // เหตุการณ์ถัดไปที่อาจไม่มา · ทางที่ตกไปเป็น `ask` ไม่ผ่านตรงนี้โดยตั้งใจ:
        // คำถามย้ายไปที่ terminal แล้ว ซึ่งแปลว่ายังรอคนอยู่จริงๆ
        store.answered(request.sessionId)
        reply(Self.answer(allow ? .allow : .deny))
        // จอต้องเห็นทันที ไม่ใช่รอจังหวะนาฬิกาถัดไป — วินาทีที่การ์ดยังค้างอยู่หลังนิ้ว
        // ยกขึ้นแล้ว คือวินาทีที่คนอ่านว่าการแตะไม่ติด
        publish()
    }
